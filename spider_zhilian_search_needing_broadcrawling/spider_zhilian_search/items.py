...
# from spider_zhilian_search.defaultpackages import TxtItemExporter
...
def clean(x):
    return x.strip('|').strip('</p>').strip("\n").strip('\t').strip('\r').strip("<p>").strip('&nbsp').strip('<br />').strip('<br/>').strip('</p>').strip(' ')
