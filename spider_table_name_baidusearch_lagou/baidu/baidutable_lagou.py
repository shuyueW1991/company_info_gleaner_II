...
class baiduProcess(object):
    ...
    def main_process(self):
        ...
        lagou_sql = "select lg_co_name from lagou_history_co"
        try:
            cursor.execute(lagou_sql)
            lagou_res = cursor.fetchall()
            for item in lagou_res:
                lagou_co_nm = item["lg_co_name"]
                # print("***********************")
                # print("\n"+lp_co_nm)
