...
class data_upload(object):
    ...
    def fetch_insert_info(self):
        ...
        fetch_sql = "select host from homepage_content_companyoritented"
        print(fetch_sql)
        try:
            ...
            while item is not None:
                ...
                print("host:")
                print(host)
                conn_fetch2 = pymysql.connect(host=self.source_mysql_host, user=self.source_mysql_user,
                                             passwd=self.source_mysql_passwd, db=self.source_mysql_db, charset='utf8')
                cursor_fetch2 = conn_fetch2.cursor(pymysql.cursors.SSDictCursor)
                fetch_sql2 = "select content from homepage_content where host='" + str(host) + "'"
                print(fetch_sql2)
                cursor_fetch2.execute(fetch_sql2)
                print("cursor2 is eksekuted...")
                item2 = cursor_fetch2.fetchone()
                content2 = ""
                while item2 is not None:
                    if item2["content"] is not None:
                        content2 = content2 + item2["content"]

                    item2 = cursor_fetch2.fetchone()
                print("the content2 is merged beautifully...")
                # print(content2)
                T_or_F_by_reSearch = self.checkif(content2)
                print("the content has been checked...")
                print(T_or_F_by_reSearch)
                print('upload those things ...')
                ...
                try:
                    ...
                    upload_language = "update homepage_content_companyoritented set content='" + content2 + "',  T_or_F_by_reSearch=' " + T_or_F_by_reSearch + "' where host='" + host +"'"
