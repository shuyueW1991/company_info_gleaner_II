...
class baiduMatchCoNames(object):
    ...
    def baidufetch_lg_ij_coinfo(self):
  ...
  #      lp_sql = "select lp_co_nm, lp_co_intro from lp_job_co_info"
        ...
        try:
            ...
            for item in ij_res:
                ...
                self.find_co_name_match(it_full_name,lg_co_dict, cursor, conn)
    ...
    def find_co_name_match(self, it_full_name, lg_co_nm_info, cursor, conn):
        find_status = 0
        print("*************************************************")
        ...
        if it_full_name.replace("+","").strip() != '':
            for lg_co_nm in lg_co_nm_info.keys():
                ...
                status = len(re.findall(it_full_name, lg_co_intro))

                if status >= 3:
                    #tmp = (it_full_name, ",".join(it_short_names), lp_co_nm)
                    #all_matches.append(tmp)
                    #match_sql = "insert into co_names_mapping(it_full_name, it_short_names, lp_co_nm) values (%s,%s,%s)"
                    match_sql = "insert into new_baidumap_ij_lg_co_names(it_full_name, lg_co_nm) values ('{0}','{1}')"
                    try:
                        #cursor.executemany(match_sql, all_matches)
                        cursor.execute(match_sql.format(it_full_name, lg_co_nm))
                        conn.commit()
                    except:
                        traceback.print_exc()
                        pass
